...
「智能家居套件」内含有各種類的電子模組和第三方感測器。配合課程，學生通過使用該套件能夠發揮創意，進行無限創作，例如製作一個環境監測裝置，又或是組裝一個智能的安防裝置……豐富的製作材料能夠更好地激發學生創作能力。其中，學生通過使用WiFi通訊模組，能夠感受物聯網中數據傳輸的神奇魅力，體驗到科技創新正在逐漸改變我們的生活方式。

![kit](../media/kit_2.png)    
\#1.[结合IFTTT](https://vimeo.com/250423195):按鈕觸發後，發送提醒事項至iOS 設備
                
![kit](../media/kit_3.png)   
\#2.[結合 MIT AI2](https://vimeo.com/250422799):通過 MIT AI2，遠距控制燈光和電機的響應

![kit](../media/kit_4.png)   
\#3.[迷你氣象監測站](https://vimeo.com/229513379):將週邊溫溼度、光照、聲音情況，呈現在熒幕上
...
CocoMod 可以讓你的作品能夠擁有感應、控制真實世界的能力，只要接上各種輸入/輸出模組或元件， 例如 LED 燈、喇叭、馬達、開關、溫溼度傳感器、聲音傳感器、紅外線、顯示裝置，或是接上 WiFi 通訊 模組，就可以做出各式各樣的應用產品或作品。

例如，利用溫度傳感器控制冷氣、用聲音控制燈光開關、 利用伺服馬達控制機器手臂、機器人，控制車、以及這些年很紅的四旋翼飛行器等等。

![kit](../media/kit_6.png)

上圖：部分用 CocoMod 電子模組製作的創意原型，電子琴、雙足機械人、迷你氣象站、穿戴式 LED 手錶

　　用户利用這些模組，結合 3D 打印和編程可以輕鬆地完成各類項目，其擴展性較市面上其他多數 Arduino 套件更適合入門者學習電子及編程知識。同時，我們鼓勵用戶通過結合線上及線下的課程，來更好、更快 的學習使用 CocoMod 電子模組

### 關於第三方感測器
為了讓用戶在使用智能家居套件時有更多樣的創作可能性，
我們提供了數 10 種第三方感測器，讓用戶有更多創作選擇
...
<table>
<tr><th colspan="2">第三方感測器（套件中各 1 個）</th></tr>
<tr><td>1. 土壤溼度感測器 </td><td>7. 超聲波距離感測器</td></tr>
<tr><td>2. 雨滴感測器 </td><td>8. 火焰感測器</td></tr>
<tr><td>3. 時鐘模塊 </td><td>9. 震動感測器</td></tr>
<tr><td>4. 煙霧感測器 </td><td>10. 紅外避障感測器</td></tr>
<tr><td>5. 激光頭模塊 </td><td>11. 傾斜感測器</td></tr>
<tr><td>6. 人體紅外感測器 </td><td>12. 光照感測器</td></tr>
</table>
...
## 使用感測器
### 使用環境感測模組
環境感測模組包含多個感測器，可以用來偵測環境相關資料，比如溫度、濕度、光照强度
和聲音强度等等。
...
2.該積木能夠讀取周圍聲音的數值，數值範圍為0~1023

![kit](../media/kit_14.png)　![kit](../media/kit_15.png)

3.該積木能夠讀取周圍光照强度的數值，數值範圍為0~1023
...
5.該積木能夠讀取周圍環境濕度的數值，濕度數值範圍是20-90% （誤差± 5%）

![kit](../media/kit_20.png)　![kit](../media/kit_19.png)

6.該積木能夠設定環境感測模組中燈光的RGB數值

![kit](../media/kit_22.png)　![kit](../media/kit_23.png)

7.該積木能夠設定環境感測模組中蜂鳴器的音調頻率

![kit](../media/kit_24.png)　![kit](../media/kit_25.png)

8.該積木能夠關閉環境感測模組中的蜂鳴器

![kit](../media/kit_26.png)　![kit](../media/kit_25.png)

#### 環境感應模組使用參考
**讀取並顯示環境參數**：使用環境感應模組，拼合OLED屏幕模組，將讀取的環境參數，例如溫度、濕度、亮度等顯示在OLED螢幕上。
...
### 使用第三方感測器
#### 使用土壤溼度感測器
土壤濕度感測器用於檢測土壤的水分，將土壤感測器插入土壤中，土壤越乾燥，輸出電壓越小，越溼潤輸出電壓越大。
...
*使用示例
1.  組裝好電子模組部分：主控模組、轉接模組
2.  拿出 USB 線連接計算機和主控模組；
3.  將「土壤溼度感測器」的接口按照下圖連接至「轉接模組」上
...
這段程式將會讀取感測器感測到的類比數值，並將數據打印在 Serial Monitor 中。數值將會在 0~1023 間變化。
#### 使用雨滴感測器
可用於不同天氣狀況的監測，並轉換成數位訊號或類比訊號；使用時，感應板上沒有水滴時，數位輸出為高電平，開關指示燈滅，滴上一滴水，數位輸出為低電平，開關指示燈亮。下面是雨滴感測器的組成部分：
...
這段程式將會讀取感測器感測到的數位數值，並將數據打印在 Serial Monitor 中。數值將會以 0 和 1呈現，代表附近有人或無人。

#### 使用超聲波距離感測器
該感測器利用超聲波測距離，多應用於機器人避開障礙物或其他距離測量的項目。感測距離的單位為 CM（厘米）
...
1.  組裝好電子模組部分：主控模組、轉接模組
2.  拿出 USB 線連接計算機和主控模組；
3.  將「超聲波距離感測器」的接口按照下圖連接至「轉接模組」上
...
這段程式將會讀取感測器感測到的距離數值（CM），並將數據打印在 Serial Monitor 中，同時用戶可以設置最遠的感測距離數值
#### 使用火焰感測器
...
這段程式將會讀取感測器感測到的類比數值，並將數據打印在 Serial Monitor 中。數值將會以 0 和 1呈現，代表感測器附屬的物體是否有震動。
...
這段程式將會讀取感測器感測到的類比數值，並將數據打印在 Serial Monitor 中。數值將會以 0 和 1呈現，代表感測器檢測的距離內是否有障礙。
#### 使用傾斜感測器
...
這段程式將會讀取感測器感測到的類比數值，並將數據打印在 Serial Monitor 中。數值將會在 0~1023 間變化。
### 瞭解數組
在計算機科學中，数组是由相同类型的元素的集合，所組成的資料。 
在 CocoBlockly 中，我們使用數組來存儲一系列數據（適用於傳輸大於 1 個數據的情況）。

![kit](../media/kit_66.png)

像上面的情況：
第一個積木為創建數組所需要的積木，其中有 3 個數據；
第二個積木為獲取數組中位置為第 0 位的數據

**註：在計算機中，位置順序由 0 開始數起。**
...
### 數據收集
本示例將向用戶展示如何收集環境感測模組的數據，並將數據打印在 Serial Monitor 中

<img src="../media/kit_68.png" width="1000px" height="350px"/>

## 使用藍牙模組
### 關於藍牙模組
**註：藍牙模組不含在智能家居套件中，如欲使用，請聯繫銷售購買或者獲取樣品**

CocoRobo 的藍牙模組為一款通訊類模組，使用藍牙 2.0 為通訊協議，能夠接收來自 Android 手機 app 通過藍牙傳輸進入的數據。
...
藍牙設定指引：
請掃描上方的二維碼下載 apk 安裝文件，並安裝。安裝後，請前往系統設定中的藍牙設備管理中，添加新設備，在確保藍牙模組和主控模組組合，以及 USB 通電的情況下，搜索名為 HC-05 的設備，並配對。連接成功後，打開名為 Coco BT Slider 的 app，點擊界面中的黃色按鈕選擇前面已經配對好的設備，進行連接。

![kit](../media/kit_74.png) 
##### 程式示例
![kit](../media/kit_75.png) 
#### 藍牙控制電機
**註：電機驅動模組不含在智能家居套件中，如欲使用，請聯繫銷售購買或者獲取樣品**

本項目結合以下模組／配件一起使用：電機驅動模組、主控模組、2 個 TT 電機

![kit](../media/kit_76.png) 

項目描述：本項目將結合特定 Android app 進行，使用 app 中的4個按鈕，來控制兩個電機的轉動方向。

![kit](../media/kit_77.png) 

藍牙設定指引：
請掃描上方的二維碼下載 apk 安裝文件，並安裝。安裝後，請前往系統設定中的藍牙設備管理中，添加新設備，在確保藍牙模組和主控模組組合，以及 USB 通電的情況下，搜索名為 HC-05 的設備，並配對。連接成功後，打開名為 Coco Robot Car  的 app，點擊界面中的黃色按鈕選擇前面已經配對好的設備，進行連接。
...
#### 藍牙控制舵機
**註：舵機轉接驅動模組不含在智能家居套件中，如欲使用，請聯繫銷售購買或者獲取樣品**

本項目結合以下模組／配件一起使用：舵機轉接模組、主控模組、1 個9g 舵機

![kit](../media/kit_80.png) 

項目描述：
本項目將結合特定 Android app 進行，使用 app 中的滑動條，控制舵機轉接模組上的所有 6 個接口的角度

![kit](../media/kit_81.png) 

藍牙設定指引：
請掃描上方的二維碼下載 apk 安裝文件，並安裝。安裝後，請前往系統設定中的藍牙設備管理中，添加新設備，在確保藍牙模組和主控模組組合，以及 USB 通電的情況下，搜索名為 HC-05 的設備，並配對。連接成功後，打開名為 Coco Servo Controller  的 app，點擊界面中的黃色按鈕選擇前面已經配對好的設備，進行連接。
...
#### 示範案例
下面我們將使用環境感測模組、主機板模組和WiFi通訊模組 實現遠距離監測環境的實時變化。

實現的效果：【CocoCloud顯示環境感測模組讀取的實時數據】
...
　　爲了更好地使用CocoBlockly編寫程式，先注冊一個CocoRobo賬號，成功注冊賬號后，可使用CocoRobo的雲端儲存功能和CocoCloud的數據可視化功能。CocoRobo賬號可以前往Cocorobo官方網站主頁注冊，登陸后，CocoCloud網站將同步登陸你的賬號。

![kit](../media/kit_90.png) 
* STEP 2 — 編寫主機板模組的程式：使環境感測模組讀取環境的數據，并且將數據發送WiFi通訊模組

請前往CocoBlockly平臺 http://cocorobo.hk/cocoblockly/dev  的主控模式進行程式編寫，然後將程式單獨上傳至主機板模組。

![kit](../media/kit_92.png) 　![kit](../media/kit_93.png) 
* STEP 3 — 前往CocoCloud，新建Event，獲取URL地址

![kit](../media/kit_91.png) 
* STEP 4 — 編寫WiFi通訊模組的程式：使WiFi通訊模組接收數據，連接網絡，并且將數據發送至CocoCloud雲平臺
![kit](../media/kit_94.png) 
* STEP 5 — 拼合環境感測模組、主機板模組和WiFi通訊模組，接通電源，觀察數據的變化
![kit](../media/kit_95.png) 
#### 參考示例
為了方便新用戶瞭解如何使用這些積木，我們提供了一些範例供參考：
...
<span style="background-color:#cc0000; color:white">示例4</span>：接入 IFTTT 的 Webhook這個示例將會以接入 IFTTT 為示例，告訴你如何發起 HTTP GET 請求，並獲得請求得到的答覆內容，打印在 Serial Monitor 中
...
### 使用主控/Wi-Fi數據通訊積木
數據傳輸積木：主控模式（Main Mode）和 WiFi 模式（WiFi Mode）下都有該類型積木，用於兩個模組之間的數據傳輸，可接收也可以發送。
#### 主控模式的數據傳輸積木
...
此類積木主要用於 CocoCloud 雲端數據的接收（解析）和發送，數據內容格式為 JSON。
...
　　ThingSpeak 是由 MathWorks 開發的一款雲端資料整合器的平台，只需要透過簡單 RESTful API 請求，用戶使用自己的 IoT 設備可以向 Thinkspeak 進行數據傳輸，同時 Thingspeak 提供足夠強大的數據分析和處理能力，比如在線編輯／調校數據，或者結合 MATLAB 進行數據可視化，是目前比較流行的在線數據分析／可視化工具之一。
