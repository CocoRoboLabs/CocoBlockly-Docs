...
#### AI人臉識別
  通過前置攝像頭抓取圖片，AI系統自動識別使用者人臉照片，判斷照片内容是笑臉還是苦臉，並將反饋結果存儲到雲端，通過WiFi模組從雲端獲取對應事件的數據，並傳輸給主機板，完成後續編程項目

#### AI語音識別

通過使用麥克風抓取使用者説話音頻，AI系統自動識別使用者説話内容，將内容存儲到雲端，通過WiFi模組從雲端獲取對應事件的數據，並傳輸給主機板，完成後續編程項目
...
b.點擊按鈕:點擊電腦前置攝像頭抓取圖片，顯示在右側，并且反饋表情檢測結果
c.事件選擇：點擊可選擇CocoCloud内已創建事件（選擇事件之前需要保證CocoBlockly賬戶登陸）
...
a.點擊按鈕:點擊電腦前置攝像頭抓取圖片，顯示在右側，并且反饋表情檢測結果
b.事件選擇：點擊可選擇CocoCloud内已創建事件（選擇事件之前需要保證CocoBlockly賬戶登陸）
...
sad face 效果展示：抓取圖片后保存圖片，并且顯示反饋信息"sad face" 
