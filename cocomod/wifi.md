...
<table width="800">
  ...
  <tr  >
    ...
    <td> a.聯網積木: 設置連接的熱點名字與密碼 <br> b.連接判斷積木: 判斷是否連接成功 <br> c.本地IP積木:獲取連接無線網絡的WIFI的IP地址</td>
  </tr>
  <tr>
...
WiFi通訊模組可以通過類積木接入現有網絡，或者建立本地網絡(AP模式)。
...
4. 獲取IP積木 : 連接網絡成功后, 可獲取WiFI通訊模組在該網絡中的IP地址
...
WiFi通訊模組與主機板模組進行數據通訊使用,即是可以向主機板模組發送數據或者從主機板模組接收數據
...
* 傳輸失敗 : 接收主機板模組傳輸的數據出錯。通常是未連接主機板模組時出現
  
![wifi__main](../media/wifi/wifi_transfer_2_receivefail.png)

3. 取值積木 : 與接收積木同用，獲取主機板傳輸過來的單個數據，以0為起始位數

* 接收主機板模組傳輸的數據，并且打印到序列埠

![wifi__main](../media/wifi/wifi_transfer_3_print_main.png)
...
WEB服務包含IFTTT，Thingspeak，CocoCloud相關積木，允許模組與這三類網絡服務交互，該單元積木的使用需要配合連接網絡積木才能成功連接到網絡服務。
...
服務端積木用於WiFi模組建立本地服務器。通過設置服務器的指定路徑，以及指定行為方式來搭建一個能夠響應的簡易的本地服務器。通常是需要與建立熱點積木一起使用
...
* 設置簡單的路徑: 設置路徑“/hi”, 當客戶端訪問該路徑時，該服務端WiFi通訊模組在序列埠打印“Hello world”
  
![wifi__main](../media/wifi/wifi_server_3_intro.png)

4.服務器發送數據積木 :

* 服務器響應發送數據 “1”: 設置路徑“/hi”, 當客戶端訪問該路徑時，該服務端WiFi通訊模組服務器向客戶端發送數據 “1”
...
* 服務器響應接收數據:設置路徑“/hi”, 當客戶端通過該路徑向該WiFi模組服務器發送數據的時候，服務器端接收數據，並存到命名為Data_receive的數組中

![wifi__main](../media/wifi/wifi_server_5_intro.png)

6.服務器接收數據的取值積木 :

* 從服務器接收的數據中獲取積木：從數組Data_receive中獲取第一位類型為數字的值
...
客戶端積木用於WiFi模組與已經建立起的本地服務器進行通訊的積木。向對應本地服務器發起請求，完成發送數據或者接收數據的操作。通常是需要與連接網絡積木一起使用，並且需要連接至本地服務器對應熱點網絡中。
...
* 向服務器請求獲取數據: 向服務器路徑為“/hi”的地址請求數據，并存到客戶端WiFi通訊模組的Data_receive數組中
...
* 向服務器發送數據： 向服務器路徑為“/hi”的地址發送數據[12]
...
網頁製作積木，用於構建簡單的網頁顯示，通常是與服務器設置主頁積木同用

##### 積木簡介  

1. 設置網址-LABEL顯示積木 :

* 設置一个网站，内容标签為“Data”，对应數據為“123456”的字符串 :

![wifi__main](../media/wifi/wifi_webpage_1_intro.png)

![wifi__main](../media/wifi/wifi_webpage_1_intro2.png)

1. 設置網址-Button設置積木 : 每一個按鈕有對應一個名稱設置，和一個路徑設置。路徑設置的是反饋路徑。指當點擊網址中顯示的該按鈕時候，觸發一個HTTP請求發送，請求對應路徑為設置的反饋路徑。發送的數據內容為當前按鈕名稱的字符串。

* 設置按鈕名稱為 “Test”，路徑為“/1” :配合使用應當設置服務器設置路徑積木，并積木的路徑應為“/1”,作為響應處理
...
#####  項目簡介 : 教學模塊控制LED模塊（WiFi對WiFi）

教學板模塊控制LED模塊：使用的是AP模式,即是通過兩個WiFi通訊模組進行數據傳輸，達到遠程控制的效果。兩個WiFi通訊模組，其中一個作為服務端，另一個作為客戶端。此項目以與教學模組拼接的WiFi通訊模組為服務端，與LED模組拼接的WIFI通訊模組為客戶端。此項目選用教學板模組上的pin 12開關，通過撥動教學板模組上pin 12的開關，來決定遠程LED燈的閉合
...
* 主機板 - 接收WiFi通訊模組的數據，設置LED
  程式下載 <a href="../xml/project1/LED_main.xml" download >Basic_main.xml</a>

![wifi__main](../media/wifi/wifi_project_1_C_main.png)

* WiFi - 設置連網，設置客戶端發起請求，發送請求到的數據到主機板
  程式下載 <a href="../xml/project1/LED_wifi.xml" download >Basic_main.xml</a>
...
網頁呈現環境模塊數據：使用的是AP模式,即是通過WiFi通訊模組設置服務器，設置主頁，在網頁上顯示環境數據。
...
* WiFi - 設置熱點，設置本地服務器，接收主機板傳輸的數據，設置主頁
...
網頁呈現環境模塊數據：使用的是AP模式,即是通過WiFi設置主頁，通過主頁上面按鈕點擊效果，達到遠程WiFi通訊模組的LED亮暗。
| 服務端 | 客戶端 | 
| :------ |:------ |
| LED模組 | 瀏覽器 |
| 主機板模組 |  |
...
* WiFi - 設置熱點，設置本地服務器，設置主頁，設置響應路徑，傳輸數據至主機板模組
