...
轉接模組上嵌有多組接口，用於多種外接設備如伺服馬達，馬達的轉接，還可以外接第三方感應器，如紅外線感應器和超聲波感應器等等。每組接口由地線（GND），電源輸出（VCC），訊號接口（Digital / Analog）組成。
...
1. 轉接模組上有較多針脚，使用時請小心謹慎，以免傷手；

2. 其中，編碼「A」和「B」指示轉接模組上提供的不同訊號接口，編碼「1」和「2」指示轉接模組兩側不同類型的拼接口，也即接入型和接出型；

3. 兩種類型拼接口的轉接模組儘在拼接口上存在區別，其他使用方法是一樣的；
...
#### 模組組裝
第三方感應器可以透過杜邦綫和轉接模組連接起來，並將轉接模組與主機板模組拼接在一起，用USB連接主機板模組至電腦，如下圖（圖中為雨滴感應器）：

<div style="padding: 10px 0 40px 0;text-align: center;"><img src="../media/hub_rain.jpg" width="70%" /></div>

每個第三方感應器使用的連接引脚都不盡相同，**關於第三方感應器的具體介紹及使用方法請參見 [使用第三方感應器](/cocomod/3rd-party)**
